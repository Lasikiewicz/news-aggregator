...
// --- CONFIGURATION ---
// Main categories defined for the application.
...
// Feeds are now mapped to these main categories.
const feeds = [
    // PlayStation
...
// --- SUB-CATEGORY KEYWORD MAPPING ---
// Used to assign sub-categories based on keywords in the article title.
const subCategoryKeywords = {
    ...
    [MAIN_CATEGORIES.XBOX]: {
        ...
        'Xbox 360': ['Xbox 360'], 'Gamepass': ['Game Pass'] // Note: "Game Pass" will not match "PC Game Pass"
    },
    [MAIN_CATEGORIES.NINTENDO]: {
        'Switch 2': ['Switch 2'], 'Switch': ['Switch'], // Order matters: Check for "Switch 2" first.
...
/**
 * Determines the primary category and sub-category based on keywords in the title.
 * @param {string} title The article title.
 * @param {string} feedCategory The default category from the feed config.
 * @returns {{category: string, subCategory: string}}
 */
function determineCategories(title, feedCategory) {
    const searchTitle = title.toLowerCase();

    // If the feed is already platform-specific, use that as the main category.
    ...
    // For multi-platform feeds, iterate through all main categories to find a match.
    for (const mainCategory of Object.values(MAIN_CATEGORIES)) {
        const subCat = findSubCategory(searchTitle, mainCategory);
        if (subCat !== 'General') {
             // Found a match in a multi-platform feed, so we can set both category and sub-category
            return { category: mainCategory, subCategory: subCat };
        }
    }
    
    // If no keywords matched from any category for a multi-platform feed.
    return { category: 'General News', subCategory: 'General' };
}

/**
 * Helper function to find a sub-category match within a specific main category.
 * @param {string} searchTitle The lowercased article title.
 * @param {string} mainCategory The main category to search within.
 * @returns {string} The found sub-category or "General".
 */
function findSubCategory(searchTitle, mainCategory) {
    ...
    // Sort sub-category keys by length descending to match more specific terms first (e.g., "Xbox Series X" before "Xbox").
...
/**
 * Extracts the main image URL using cheerio.
...
/**
 * Processes a single article: categorizes it, asks AI to rewrite content, and saves to Firestore.
 * @param {object} item The article item from the RSS feed.
 * @param {object} feedInfo Contains metadata about the feed.
 */
const processArticle = async (item, feedInfo) => {
    ...
    // Determine category and sub-category using keyword logic BEFORE calling the AI.
